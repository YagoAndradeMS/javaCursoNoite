...
public class Tela
{
 ...
 public void imagem(String arquivo, int xa, int ya, int larg, int alt, double dir, double x, double y) {
	    ...
	    if (!sprites.containsKey(arquivo)) {
	        try {
	            // Carrega a imagem a partir do pacote do projeto
	            sprites.put(arquivo, ImageIO.read(getClass().getResourceAsStream("/Images/" + arquivo)));
	        } catch (java.io.IOException ioex) {
	            throw new RuntimeException(ioex);
	        }
	    }

	    // Pega a imagem carregada
//	    BufferedImage img = sprites.get(arquivo);
	    
	    // Realiza operações de rotação e desenho da imagem
